...
LDRB R1, [R0], #1 ; get the first byte
ADR R2, bmask0 ; get address of first mask
LDRB R3, [R2], #1
MOV R4, #1 ; current position in byte
MOV R6, #0  ; current byte
loop
AND R5, R1, R3
CMP R5, R3 ; IF value is 1 change something
BEQ out_black
BNE out_white
...
ADD R7, R7, R6
ADD R7, R7, R6
ADD R7, R7, R6 ; add current byte 3 times
MOV R8, R4
ADD R8, R8, #-1
LDR R9, LCD_width
MUL R8, R8,R9
